...
def prefix_test_set_ids(filename: str, batch_num: Optional[int] = None):
    ...
    Args:
        filename: 原始测试集文件名 (如 test_set_B01.json)。
    ...
    # 将修复后的数据保存为新的文件
    new_filename = f"test_set_{batch_prefix}_fixed.json"
...
def process_all_test_sets(pattern: str = "test_set_B*.json"):
    """
    处理匹配模式下的所有测试集文件（如 test_set_B01.json, test_set_B02.json, ...）。
    """
